# このBOTについて
このBOTは[serval-hubot](https://github.com/sifue/serval-hubot)から入退室メッセージ機能以外を削除したBOTです。
オリジナル版から少しコードに変更を加えています。

# join-leave-hubot の機能 (scripts/chat.js)

- 「入室メッセージ登録 %USERNAME%さんいらっしゃい。ここは、%ROOMNAME% です。」のように発言すると「%USERNAME%さんいらっしゃい。ここは、%ROOMNAME% です。」をそのチャンネルの入室メッセージとして登録します。
- 入室メッセージの `%USERNAME%` は表示名またはユーザー名に、 `%ROOMNAME%` は #programming のようなチャンネル名に置換されます。
- 「入室メッセージ削除」のように発言すると、そのチャンネルの入室メッセージを解除します。

# 以下 join-leave-hubot の Hubot としての説明
